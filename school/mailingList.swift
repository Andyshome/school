...
class mailingList: UIViewController, UITableViewDataSource, UITableViewDelegate, MFMailComposeViewControllerDelegate ,UISearchBarDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        myCell.textLabel?.text = nameFit[indexPath.row].name
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        nameFit = Teacher.filter({teacher -> Bool in
    ...
    override func viewDidLoad() {
        ...
        for index in 0..<names.count {
            Teacher.append(teachersProfile.init(name: names[index], emailAddress: emails[index]))
        }
        nameFit = Teacher
